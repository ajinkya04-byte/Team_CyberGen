...
def save_json_report(results, open_ports, closed_ports, filtered_ports, service_versions, os_info, target_url):
    report = {
        "target": target_url,
        "vulnerabilities": [
    ...
    file_name = input("Enter file name for JSON report (without extension): ").strip()
    if not file_name:
        file_name = "scan_report"
    ...
    try:
        ...
        print(f"JSON report saved as {file_path}")
        return file_path   # 👈 FIX: return path
    except Exception as e:
        print(f"Error saving JSON: {e}")
        return None


def process_with_ai(json_file, model="llama3"):
    import requests

    print(f"[DEBUG] process_with_ai received: {json_file}")
    if not json_file or not os.path.exists(json_file):
        print("❌ No JSON report to process with AI.")
        return
    ...
    * **Add a comment to each code snippet explaining what it does and why it's a fix.**
    ...
    try:
        response = requests.post(
            ...
            json={
                "model": model,       # e.g. "llama3", "mistral", "codellama"
                "prompt": prompt,
                "stream": False
            }
        )
        ...
        print("\n✅ AI analysis saved as ai_analysis.json")
...
def start_scan():
    ...
    process_with_ai(json_file, model="mistral")
